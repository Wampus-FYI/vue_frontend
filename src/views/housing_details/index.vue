  ...
  <div class="listing-container" v-if="housing">
    ...
    >
      <el-tab-pane 
        ...
        <div v-else>
        <apexchart width="800" type="bar" :options="apexGraphValues[label].options" :series="apexGraphValues[label].series"></apexchart>
        </div> 
      </el-tab-pane>
    </el-tabs>
  ...
  Object.keys(housing.value.Rent).forEach(year => {
        ...
        for (let row = 0; row < housing.value.Rent[year].length; row++) {
            for (let col = 0; col < housing.value.Rent[year][row]?.length; col++) {
                ...
                if(housing.value.Rent[year][row][col] !== null) transformedData[year][key] = housing.value.Rent[year][row][col][0];
            }
        }
    });
...
const swiperOptions = computed(() => {
      return {
        ...
        navigation: housing.value.Photos?.length > 1 ? {
          nextEl: '.swiper-button-next',
          prevEl: '.swiper-button-prev',
        } : false,
        pagination: housing.value.Photos?.length  > 1 ? {
          el: '.swiper-pagination',
          clickable: true,
          dynamicBullets: true,
        } : false,
        autoplay: housing.value.Photos?.length  > 1 ? {
          delay: 2500,
